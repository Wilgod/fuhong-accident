...
export async function postCMSWorkflowGetUserInformationId(context: WebPartContext, serviceLocation:string, workflowUrl:string) {
    try {
        const CONFIG: ISPHttpClientOptions = {
            headers: {
                "Accept": "application/json;odata=verbose",
...
export async function postCMSUserInformationWorkflowGetUser(context: WebPartContext, UserinformationId:string, workflowUrl:string) {
    try {
        const CONFIG: ISPHttpClientOptions = {
            headers: {
                "Accept": "application/json;odata=verbose",
                ...
                __metadata: { type: 'SP.Data.TestListItem' },
                "cr98a_userinformationid": UserinformationId
            })
        };
        let userList = await context.httpClient.post(workflowUrl, SPHttpClient.configurations.v1, CONFIG).then((response: SPHttpClientResponse) => {
            return response.json();
